class Team:
    ...
    def setRankingPoints(self):
        self.beltGP=self.beltWins+self.beltTies+self.beltLosses
        self.beltWP=(self.beltWins+self.beltTies/2)/(self.beltGP) if self.beltGP>0 else 0
        self.rankingPoints+=100*self.numReigns
        self.rankingPoints+=10000*self.beltWins
        self.rankingPoints+=(self.beltWP/1000)
        if self.beltWins==0:
            self.rankingPoints=self.beltTies+self.beltLosses/1000

    def getBeltRecord(self):
        if self.currentReign!=None:
            for game in self.currentReign.games:
                self.beltWins+=1
    ...
    def rankSummary(self,rank):
        ...
        if self.beltTies!=0:
            wlt="%s-%s-%s" % (self.beltWins,self.beltLosses,self.beltTies)
        else:
            wlt="%s-%s" % (self.beltWins,self.beltLosses)
